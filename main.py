...
async def chat(request: ChatRequest):
    ...
    너는 법률 검색어 최적화 AI야. 
    [초중요] 국가법령정보센터 검색 엔진은 융통성이 없어서 단어가 많으면 검색을 못 해!
    검색어는 반드시 "[정확한 법령명] [가장 핵심이 되는 명사 단 1~2개]" 조합으로만 만들어!
    "규정", "기준", "방법" 같은 쓸데없는 단어는 무조건 빼!
    
    예시 1: "근로기준법상 연차 휴가 규정" -> "근로기준법 연차" (규정이라는 단어 제거)
    예시 2: "건축법상 일반음식점 허가기준" -> "건축법 일반음식점"
    예시 3: "민방위 면제는 몇살부터?" -> "민방위기본법 면제"
    """
    try:
